  ...
  const updateStoredTransaction = async (transactions, updatedTransaction) => {
    ...
    try {
      ...
      const list = transactions;
        ...
        setCurrentTransaction(searchByID(transactions[pos].id, storageObj.transactions));
    ...
    showMessage({
          ...
          // onPress: () => {
            // THIS FUNC/CB WILL BE CALLED AFTER MESSAGE IS PRESSED 
          // },
  ...
  const updateTransactionCategory = async (category) => {
    ...
    try {
      ...
      if (found) {
        ...
        if (currentTransaction.type.toLowerCase() === 'income' && currentTransaction.amount < 0) {
          currentTransaction.amount = currentTransaction.amount * -1;
        } else if (currentTransaction.type.toLowerCase() === 'expense' && currentTransaction.amount >= 0) {
          currentTransaction.amount = currentTransaction.amount * -1;
        }
        ...
        // updateStoredTransaction(storageObj.transactions, currentTransaction);
      }
    } catch (e) {
      // statements
    ...
    // showMessage('Successfully updated transaction category');
  };
...
// export default Home; // TransactionsList; // Home;a
...
//             01/01/2020 03:28 PM | AppSync Settings
//             01/04/2020 08:25 AM | Released version 1.1.0 to App Store!
//             02/04/2020 05:50 AM | Added SoundF
//             02/28/2020 02:34 PM | Enabling settings title
//             03/02/2020 12:25 PM
//             03/05/2020 10:06 AM | Published 2.1.0
//             03/05/2020 02:01 PM | Storybooks
...
//     updatedTransaction.version += 1;
//     // console.log(updatedTransaction.version);
//     try {
//       const storageObj = await loadSettingsStorage(global.storageKey);
...
//     // console.log(transaction);
//     // load stored user transactions
//     transaction.payee = new Payee(string)
...
//       // showFlashMessage(`Logged in as ${cognito.attributes.email}`);
//       global.storageKey = cognito.attributes.sub
...
//     setCurrentTransactions(userObject.transactisons);
...
//       // backgroundColor: colors.dark, // "purple", // background color
//       color: 'white', // "#606060", // text color
//       position: 'bottom', // "top" (default), "bottom", "center"
//       // icon: { icon: 'auto', position: "right" }, // "none" (default), "auto" (guided by type)
//       // onPress: () => {
//         // THIS FUNC/CB WILL BE CALLED AFTER MESSAGE IS PRESSED 
//       // },
...
//   };
//   async function clearState() {
//     // setFlashMessageText('Hello World');
//     // showFlashMessage('Hello');
...
//       // showFlashMessage(`${currentTransactions.length} transactions found`);
//     }
//   }, [currentTransactions]);
...
//   //     <Button
//   //       onPress={() => {
//   //         // HERE WE SHOW OUR FIRST MESSAGE
//   //         showMessage({
...
//       top={top}
//       setTop={setTop}
//       isNoteInputEditable={isNoteInputEditable}
//       setIsNoteInputEditable={setIsNoteInputEditable}
//     />
//   );
